...
var versionCommand = &cobra.Command{
	Use:   "version",
	Short: "打印当前可执行文件版本并退出。",
	Run: func(cmd *cobra.Command, _ []string) {
		fmt.Printf("wings v%s\n版权所有 © 2018 - %d Dane Everitt 和贡献者\n", system.Version, time.Now().Year())
	},
}

func Execute() {
	if err := rootCommand.Execute(); err != nil {
		log2.Fatalf("执行命令失败: %s", err)
	}
}

func init() {
	rootCommand.PersistentFlags().StringVar(&configPath, "config", config.DefaultLocation, "设置配置文件的位置")
	rootCommand.PersistentFlags().BoolVar(&debug, "debug", false, "在 debug 模式下运行 wings")
...
func rootCmdRun(cmd *cobra.Command, _ []string) {
	...
	if s, err := cron.Scheduler(cmd.Context(), manager); err != nil {
		log.WithField("error", err).Fatal("未能初始化 cron 系统")
	} else {
		log.WithField("subsystem", "cron").Info("启动 cron 进程")
		s.StartAsync()
	}
	...
	go func() {
		...
		if err := sftp.New(manager).Run(); err != nil {
			log.WithError(err).Fatal("初始化 sftp 服务器失败")
			return
		}
	}()

	go func() {
		log.Info("更新面板上的服务器状态：将安装/恢复服务器标记为正常")
		...
		if err := pclient.ResetServersState(cmd.Context()); err != nil {
			log.WithField("error", err).Error("无法在面板上重置服务器状态：某些实例可能意外地陷入安装/恢复状态")
		}
	}()
	...
	if err := os.MkdirAll(sys.ArchiveDirectory, 0o755); err != nil {
		log.WithField("error", err).Error("创建存档目录失败")
	}

	// Ensure the backup directory exists.
	if err := os.MkdirAll(sys.BackupDirectory, 0o755); err != nil {
		log.WithField("error", err).Error("创建备份目录失败")
	}
	...
	if autotls {
		...
		log.WithField("hostname", tlshostname).Info("Web 服务器现在正在侦听并启用自动 TLS； 证书将由 Let's Encrypt 自动生成")
		...
		if err := s.ListenAndServeTLS("", ""); err != nil {
			log.WithFields(log.Fields{"auto_tls": true, "tls_hostname": tlshostname, "error": err}).Fatal("使用 auto-tls 配置 HTTP 服务器失败")
		}
		return
	}
	...
	if api.Ssl.Enabled {
		if err := s.ListenAndServeTLS(api.Ssl.CertificateFile, api.Ssl.KeyFile); err != nil {
			log.WithFields(log.Fields{"auto_tls": false, "error": err}).Fatal("配置 HTTPS 服务器失败")
		}
		return
	}
	s.TLSConfig = nil
	if err := s.ListenAndServe(); err != nil {
		log.WithField("error", err).Fatal("配置 HTTP 服务器失败")
	}
}
...
func initConfig() {
	...
	if err != nil {
		...
		log2.Fatalf("cmd/root: 读取配置文件时发生错误: %s", err)
	}
...
func initLogging() {
	...
	if err := os.MkdirAll(path.Join(dir, "/install"), 0o700); err != nil {
		log2.Fatalf("cmd/root: 无法创建安装目录路径: %s", err)
	}
	p := filepath.Join(dir, "/wings.log")
	w, err := logrotate.NewFile(p)
	if err != nil {
		log2.Fatalf("cmd/root: 创建 wings 日志失败: %s", err)
	}
